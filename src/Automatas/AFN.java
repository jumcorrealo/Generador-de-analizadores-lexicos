...
public class AFN {
    ...
    public void initializeAFNwithData(ArrayList<Character> sigma, ArrayList<String> states, String q,
            ArrayList<String> finalStates, ArrayList<String>[][] delta) {
    ...
    public int getColumn(String symbol) {
        // esta función es para obtener la columna en la que se encuentra un simbolo (se
        // asume fila 0 )
        for (int i = 0; i < this.sigma.size(); i++) {
            // solo nos interesa la los elementos de la primera fila entonces por eso la
            // fijamos en [0][i]
    ...
    public boolean procesarCadenaAFN(String cadena, String estado, int letra) {
        ...
        if (!this.delta[posState][posSymbol].isEmpty()) {
            ...
            for (i = 0; i < this.delta[posState][posSymbol].size(); i++) {
                ...
                // importante no dejar la impresion fuera del for o nada fuera del for porque a
                // veces se lo traga el vacio
